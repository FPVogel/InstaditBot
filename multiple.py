...
def addaccount():
    ...
    newsubreddit = str(input("What Subreddit? (without /r/) "))
    config.update({newusername: {}})
    config[newusername].update({"username": newusername})
    config[newusername].update({"password": newpassword})
    config[newusername].update({"subreddit": newsubreddit})
